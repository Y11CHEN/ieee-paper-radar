...
def test_summarize_papers_returns_summaries(mock_cls):
    ...
    mock_client.models.generate_content.return_value = MagicMock(
        text='[{"doi": "10.1109/TPEL.2024.001", "contribution": "Proposes a 2-phase SC converter with 98.5% efficiency for 48V data center bus.", "stars": "⭐⭐"}]'
    )

    result = summarize_papers(SAMPLE_PAPERS, api_key="test-key")
    assert len(result) == 1
...
def test_summarize_papers_empty_input(mock_cls):
    result = summarize_papers([], api_key="test-key")
    assert result == []
...
def test_analyze_trends_returns_text(mock_cls):
    ...
    mock_cls.return_value = mock_client
    mock_client.models.generate_content.return_value = MagicMock(
        text="**Evolution:** SC converters have moved from...\n\n**Direction 1:** Soft charging..."
    )

    result = analyze_trends(SAMPLE_PAPERS, HISTORICAL_PAPERS, api_key="test-key")
...
def test_recommend_papers_returns_tiers(mock_cls):
    ...
    mock_cls.return_value = mock_client
    mock_client.models.generate_content.return_value = MagicMock(
        text='[{"doi": "10.1109/TPEL.2024.001", "tier": "强烈推荐", "reason": "直接推进 RSCC 拓扑"}, {"doi": "10.1109/ECCE.2024.002", "tier": "跳过", "reason": "控制层面工作，非拓扑"}]'
    )

    result = recommend_papers(SAMPLE_SUMMARIZED, SAMPLE_PROFILE, api_key="test-key")
    assert len(result) == 2
...
def test_recommend_papers_preserves_existing_fields(mock_cls):
    ...
    mock_cls.return_value = mock_client
    mock_client.models.generate_content.return_value = MagicMock(
        text='[{"doi": "10.1109/TPEL.2024.001", "tier": "值得一读", "reason": "相关"}, {"doi": "10.1109/ECCE.2024.002", "tier": "跳过", "reason": "不相关"}]'
    )
...
def test_recommend_papers_empty_input(mock_cls):
    result = recommend_papers([], SAMPLE_PROFILE, api_key="test-key")
    assert result == []
    mock_cls.return_value.models.generate_content.assert_not_called()
